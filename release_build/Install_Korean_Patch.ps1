...
# Place this installer in the folder containing "Ultima I Unity.exe".
...
foreach ($f in $manifest.binary_files) {
    ...
    if ($hash -ne $f.original_sha256) {
        throw "지원하지 않는 게임 버전 또는 변경된 파일입니다: $($f.path)`n현재 SHA256: $hash`n최신 원본 게임 파일로 다시 설치/업데이트한 뒤 v0.4 패치를 적용하세요."
    }
...
foreach ($f in $manifest.json_files) {
    ...
    $hash = Hash-File $path
    if ($hash -ne $f.original_sha256) {
        throw "지원하지 않는 게임 버전 또는 변경된 JSON입니다: $($f.path)`n현재 SHA256: $hash`n최신 원본 게임 파일로 다시 설치/업데이트한 뒤 v0.4 패치를 적용하세요."
    }
    Backup-Once $path $manifest.version
...
Write-Host "v0.4 원본 백업은 각 파일 옆의 .u1k-original-v0.4 파일로 보관했습니다."

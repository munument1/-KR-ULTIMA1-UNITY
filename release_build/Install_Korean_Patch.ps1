...
function Backup-Once([string]$Path, [string]$Version) {
    $bak = $Path + ".u1k-original-v" + $Version
...
function Set-JsonValue($Root, $PathParts, $Value) {
    ...
    for ($i = 0; $i -lt $PathParts.Count - 1; $i++) {
        $part = $PathParts[$i]
        if ($part.kind -eq 'index') { $node = $node[[int]$part.value] }
        else {
    ...
    if ($last.kind -eq 'index') { $node[[int]$last.value] = $Value }
    else {
...
foreach ($f in $manifest.binary_files) {
    ...
    $hash = Hash-File $path
    if ($hash -eq $f.patched_sha256) { Write-Host "이미 패치됨: $($f.path)" -ForegroundColor DarkGray; continue }
    if ($hash -ne $f.original_sha256) {
        throw "지원하지 않는 게임 버전 또는 변경된 파일입니다: $($f.path)`n현재 SHA256: $hash`n2026-09-13 최신 원본 게임 파일로 다시 설치/업데이트한 뒤 v0.5 패치를 적용하세요."
    }
    Backup-Once $path $manifest.version
...
foreach ($f in $manifest.json_files) {
    ...
    if (Test-Path -LiteralPath $marker) { Write-Host "이미 패치됨: $($f.path)" -ForegroundColor DarkGray; continue }
    $hash = Hash-File $path
    if ($hash -ne $f.original_sha256) {
        throw "지원하지 않는 게임 버전 또는 변경된 JSON입니다: $($f.path)`n현재 SHA256: $hash`n2026-09-13 최신 원본 게임 파일로 다시 설치/업데이트한 뒤 v0.5 패치를 적용하세요."
    }
    Backup-Once $path $manifest.version
...
Write-Host "v0.5 원본 백업은 각 파일 옆의 .u1k-original-v0.5 파일로 보관했습니다."
